...
your Riak cluster (which is different from [recovering a failed node](/riak/kv/2.2.3/using/repair-recovery)). Here is the recommended way to go
...
3. Start the new `riak7` node with [`riak start`](/riak/kv/2.2.3/using/admin/riak-cli/#start):
...
participating in the cluster; for example `riak0` with the [`riak-admin cluster join`](/riak/kv/2.2.3/using/admin/riak-admin/#cluster) command executed on the new `riak7` node:
...
`riak7` node using the [`riak-admin cluster replace`](/riak/kv/2.2.3/using/admin/riak-admin/#cluster) command:
...
6. Examine the proposed cluster changes with the [`riak-admin cluster plan`](/riak/kv/2.2.3/using/admin/riak-admin/#cluster) command executed on the new
`riak7` node:
...
[`riak-admin cluster commit`](/riak/kv/2.2.3/using/admin/riak-admin/#cluster) command:

    ```bash
    riak-admin cluster commit
    ```

    If you need to clear the proposed plan and start over, use [`riak-admin cluster clear`](/riak/kv/2.2.3/using/admin/riak-admin/#cluster):
...
with the [`riak-admin ringready`](/riak/kv/2.2.3/using/admin/riak-admin/#ringready)
and [`riak-admin member-status`](/riak/kv/2.2.3/using/admin/riak-admin/#member-status)
commands.
